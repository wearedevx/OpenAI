...
public struct ChatQuery: Equatable, Codable, Streamable {
    ...
    public enum ResponseFormat: Codable, Equatable {
        ...
        public static func == (lhs: ResponseFormat, rhs: ResponseFormat) -> Bool {
            ...
            case let (.jsonSchema(lhsName, lhsType), .jsonSchema(rhsName, rhsType)):
    ...
    private struct JSONSchema: Encodable {
        ...
        public func encode(to encoder: any Encoder) throws {
            ...
            try container.encode(PropertyValue(from: schema), forKey: .schema)
        }
    }
    ...
    private indirect enum PropertyValue: Codable {
        ...
        func encode(to encoder: Encoder) throws {
            ...
            switch self {
            case let .simple(type, isOptional):
                if isOptional {
            ...
            case let .enum(cases, isOptional):
                if isOptional {
                    try container.encode([SimpleType.string.rawValue, SpecialType.null.rawValue], forKey: .type)
                } else {
                    try container.encode(SimpleType.string.rawValue, forKey: .type)
            ...
            case let .object(object, isOptional):
                if isOptional {
                ...
                let fields = object.map { key, _ in key }
                try container.encode(fields, forKey: .required)
            case let .array(items, isOptional):
                if isOptional {
                ...
                if let displayStyle = unwrappedMirror.displayStyle {
                    ...
                    case .collection:
                        if let child = unwrappedMirror.children.first {
                            self = try .array(Self(from: child.value), isOptional: isOptional)
                            return
                        } else {
